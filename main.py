...
def analyze_stock(ticker, current_price_cache):
    ...
    try:
        ...
        if equity > 0:
            ...
            # --- 3年モデル用倍率へ変更 ---
            roe_val = pseudo_roe
            mult = 1.1 # ROE 10%未満 (1.05^3 approx 1.15)
            roe_cls = "10%未満"
            
            if roe_val >= 0.20:
                mult = 1.8 # ROE 20%以上 (1.22^3 approx 1.8)
                roe_cls = "20%以上"
            elif roe_val >= 0.15:
                mult = 1.6 # ROE 15-20% (1.17^3 approx 1.6)
                roe_cls = "15-20%"
            elif roe_val >= 0.10:
                mult = 1.3 # ROE 10-15% (1.10^3 approx 1.3)
                roe_cls = "10-15%"
